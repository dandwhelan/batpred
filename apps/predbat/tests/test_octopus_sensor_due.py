...
The intelligent dispatch sensor is republished every 2 minutes. The component
scheduler (ComponentBase.start) is not minute-aligned: it sleeps in 5 second
chunks and does not account for the time run() itself takes, so run()
invocations drift relative to the wall clock. A refresh gate keyed off
wall-clock even minutes (count_minutes % 2 == 0) can therefore land only on odd
minutes and starve the sensor indefinitely. The gate must instead be based on
the age of the last sensor update.

The dispatch fetch itself is an API call and so follows the configurable
device refresh cadence (octopus_api_device_refresh_minutes); these tests
configure it to 2 minutes so both cadences can be checked together.
"""
...
async def test_octopus_sensor_due(my_predbat):
    ...
    api = OctopusAPI(my_predbat, key="test-key", account_id="test-account", automatic=False, device_refresh_minutes=2)
    ...
    # Everything else the run loop can reach is stubbed so no API or storage calls are made
    ...
    try:
        ...
        api.device_fetched_at = None
        api.session_fetched_at = base
