...
""".format(
    ...
    expected_service = [
        # Notifications are off for this test (see set_event_notify above), so only the join call
        # itself is expected here - the new Power Down service name, per the merged join logic in
        # octopus.py that tries it before falling back to the deprecated saving-session name.
...
def test_saving_session_bad_slot(my_predbat):
    ...
    rate_replicate = {}
    my_predbat.load_saving_slot(slots, my_predbat.rate_import, export=False, rate_replicate=rate_replicate)
