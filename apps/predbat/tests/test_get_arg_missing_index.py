...
back to the 8 kWh default). A value that is present but genuinely unparseable ("abc") is a
different fault and must still be reported - unlike a None index-miss, and unlike a live entity
reporting "unavailable"/"unknown" (a car that is asleep, say), which get_arg_unavailable in
test_fetch_config_options.py covers separately.
"""
...
def test_get_arg_missing_index_uses_default_quietly(my_predbat):
    ...
    try:
        ...
        my_predbat.args["soc_max"] = [25.68, "abc"]
        my_predbat.had_errors = False
        my_predbat.current_status = ""
        value = my_predbat.get_arg("soc_max", default=0.0, index=1)
        if value != 0.0:
        ...
        my_predbat.args["battery_scaling_last_known"] = [1, "abc"]
        my_predbat.had_errors = False
        my_predbat.current_status = ""
        value = my_predbat.get_arg("battery_scaling_last_known", default=2, index=1)
        if value != 2:
