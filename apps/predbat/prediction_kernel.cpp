...
// search over doubles instead of a round_py. Upstream added this when round_py was snprintf+strtod
// at ~96ns - the most expensive thing in the loop, and unusable under threading because it
// serialises on the C library's global locale. This fork's round_py takes the exact-integer fast
// path above at ~7ns and touches no locale, so the table is no longer load-bearing for speed. It is
// kept because it is pinned equivalent by pk_verify_soc_percent_table and removing it would be a
// behaviour change to justify on its own, not a side effect of a merge.
...
static int32_t pk_run_one(const ContextStore *store, const PkScenario *s, PkResult *out, int32_t soc_range_start_step, int32_t soc_range_end_step, double *soc_range_min_out, double *soc_range_max_out, PkScratch &scratch)
{
    ...
    for (int32_t k = 0; k < n_steps; k++) {
        ...
        // The rounding is only paid when something actually consumes the value. soc itself is never
        // rounded - the result feeds soc_out and the SoC range and nothing else - so skipping it
        // when a batched scenario wants neither cannot change the simulation. Upstream measured
        // this skip at -13% when round_py was snprintf+strtod at ~96ns; on this fork's ~7ns
        // exact-integer round_py the saving is far smaller, but it is still strictly free.
        ...
        if (!c->set_export_freeze_only && export_window_active && export_limit_now < PK_EXPORT_LIMIT_FREEZE && (soc > discharge_min)) {
            ...
            //
            // Freeze Export is the same inverter mode with charging disabled: execute.py sets the
            ...
            if (diff > 0) {
                ...
                // Freeze Export disables charging, so the battery holds rather than absorbing the
                // surplus - the #4207 recapture below is the only way it charges, and only for the
                // part of the surplus the export limit cannot take.
                const double charge_rate_scale = freeze_export ? 0.0 : 1.0;
                ...
                if (inverter_hybrid) {
                    ...
                    const double charge_rate_now_curve_dc_step = charge_rate_now_curve_dc * step * charge_rate_scale;
                    ...
                    battery_draw = std::max({diff, -charge_rate_now_curve_step * charge_rate_scale, -inverter_limit, -battery_to_max});
                }
            ...
            if (freeze_export) {
                ...
                // discharge during Freeze Export instead of covering house load. Feed the
                // battery-side rate into the normal AC balance so load consumes it first and any
                // surplus may export. Configuring this rate says the inverter leaks only this much
                // rather than covering load, so it replaces the shortfall discharge above.
