...
def test_discharge_target_skipped_for_ac_coupled(test_name, ha, inv, dummy_rest):
    ...
    try:
        ...
        for model in ["Hybrid", ""]:
            ...
            dummy_rest.clear_queue()
            dummy_rest.rest_data = copy.deepcopy(inv.rest_data)
            # This fork suppresses a retry once a target has been seen not to read back
            # (rest_discharge_target_unwritable), and the first iteration's write does fail against
            # the dummy - so without clearing it the second iteration would be skipped by the retry
            # suppression rather than by the model check this test is about.
            inv.base.rest_discharge_target_unwritable = {}
            inv.adjust_force_export(True, ts, te)
            commands = dummy_rest.get_commands()
