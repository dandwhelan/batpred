...
class WebInterface(ComponentBase):
    ...
    def get_power_flow_diagram(self):
        ...
        # is already inside load_power, so subtract it and the House pill reads as the rest of the
        ...
        # would take the car off a figure that never held it - leave the reading alone and draw the
        # car fed from the Grid instead (#4788).
        ...
        car_from_house = car_configured and car_inside_clamp
        house_power = max(0, load_power - car_power) if car_from_house else load_power
        car_charging = car_configured and car_power >= 10
        house_consuming = house_power > 10
        ...
        icon_car = '<path d="M -6.5,1.5 V -1 L -4.5,-4.5 H 4.5 L 6.5,-1 V 1.5 Z" fill="none" stroke="#fff" stroke-width="1.5" stroke-linejoin="round"/><circle cx="-3.5" cy="2.5" r="1.6" fill="#fff"/><circle cx="3.5" cy="2.5" r="1.6" fill="#fff"/>'
        ...
        if car_configured:
            if car_from_house:
                d_car = "M {},{} L 180,352 L 112,368".format(junction[0], junction[1])
            else:
                d_car = "M 440,311 L 300,392 L 112,381"
            flows += flow_path(d_car, CAR, car_charging, css="pf-car")
        ...
        labels += pill(255, 402, HOUSE, icon_home, "{} W".format(dp0(abs(house_power))))
        ...
        if car_configured:
            flows += dot(112, 368 if car_from_house else 381, CAR, css="pf-car")
            labels += pill(72, 318, CAR, icon_car, "{} W".format(dp0(abs(car_power))), css="pf-car")
        ...
        desc = "Power flow. Solar {} watts. House load {} watts. Battery {} percent, {} {} watts. Grid {} {} watts.".format(dp0(abs(pv_power)), dp0(abs(house_power)), dp0(soc_percent), batt_state, dp0(abs(battery_power)), grid_state, dp0(abs(grid_power)))
        if car_configured:
            desc += " Car {} {} watts, fed from the {}.".format("charging" if car_charging else "idle", dp0(abs(car_power)), "house" if car_from_house else "grid")
...
function pbChartSize() {
    ...
    if (width <= 700) {
        // Mobile: fill the viewport width and use a squarer aspect ratio. The desktop
        // floor below is deliberately not applied here - on a narrower phone it is what
        // pushes the chart into horizontal scroll, which the mobile branch exists to avoid.
        width = width - 20;
...
function pbChartSize() {
    if (window.innerWidth <= 700) {
        // Mobile: use the full viewport width rather than two thirds. Deliberately not
        // floored at 600px the way the desktop path below is - on a narrower phone that
        // floor is what puts the chart into horizontal scroll.
...
"""
    ...
    def render_heatmap_chart(self, series_data, title, range_min, range_max, chart_id="chart", fixed_height=None):
        ...
        if fixed_height is not None:
            ...
            height = max(len(series_data), 1) * 80 + 80
        ...
        text += "    return {{width: width, height: height_{}}};\n".format(js_id)
        text += "}\n"
        ...
        text += "chart_{}.render();\n".format(js_id)
        text += "pbRegisterChart(chart_{}, pbChartSize);\n".format(js_id)
        text += "})();\n"
        text += "</script>\n"
        return text
