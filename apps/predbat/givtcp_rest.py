...
class GivTCPRest:
    ...
    def write_acknowledged(self, response):
        """
    ...
    def set_discharge_target(self, target):
        ...
        # Report it once, then leave that target alone until a different one is asked for.
        if not hasattr(self.base, "rest_discharge_target_unwritable"):
            self.base.rest_discharge_target_unwritable = {}
        if self.base.rest_discharge_target_unwritable.get(inverter.id) == target:
            self.base.log("GivTCP: Inverter {} export target register does not read back {}, leaving it alone".format(inverter.id, target))
            return False
        ...
        for retry in range(INVERTER_MAX_RETRY_REST):
            ...
            # On some inverters neither source ever carries this register - it is absent from
            # GivTCP's read map and stays at 0 whatever is written - so no readback can confirm a
            # write that did in fact land. There the POST response is the only confirmation there is.
            if result == target or self.write_acknowledged(r):
                inverter.count_register_writes += 1
                self.base.rest_discharge_target_unwritable.pop(inverter.id, None)
                self.base.log("GivTCP: Inverter {} Set export target slot 1 {} via REST successful after retry {}".format(inverter.id, data, retry))
                return True
            inverter.sleep(2)

        self.base.rest_discharge_target_unwritable[inverter.id] = target
