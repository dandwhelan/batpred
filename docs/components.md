...
| `tariff_refresh_minutes` | Integer | No | 30 | `octopus_api_tariff_refresh_minutes` | How often (in minutes) to refresh account and tariff discovery from the Octopus API (minimum 2) |
| `device_refresh_minutes` | Integer | No | 10 | `octopus_api_device_refresh_minutes` | How often (in minutes) to refresh Intelligent Octopus device dispatches (minimum 2, only polled on an intelligent tariff) |
| `session_refresh_minutes` | Integer | No | 30 | `octopus_api_session_refresh_minutes` | How often (in minutes) to refresh saving sessions and free electricity events (minimum 2) |
...
Conversely, set `octopus_api_device_refresh_minutes: 2` if you are on Intelligent Octopus and want new
dispatch slots picked up within a couple of minutes rather than the default ten. The dispatch sensor
itself is republished from cached data every 2 minutes regardless, so this setting only controls how
often Predbat calls the API.
