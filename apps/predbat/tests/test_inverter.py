...
def test_discharge_target_unreadable_register(test_name, ha, inv, dummy_rest):
    ...
    REST now lives in the GivTCP component rather than on Inverter, so this drives the client's
    set_discharge_target() directly - that is where the retry suppression lives.
    """
    failed = False
    print("Test: {}".format(test_name))
    ...
    def setup_rest_case(current_target):
        """REST v3 path with the register reading back current_target."""
        inv.rest_api = "dummy"
        inv.rest_v3 = True
        inv.rest_data = {
            "Control": {"Enable_Discharge_Schedule": "on", "Mode": "Timed Export"},
            "Timeslots": {"Discharge_start_time_slot_1": "03:33:00", "Discharge_end_time_slot_1": "04:44:00"},
    ...
    try:
        ...
        if not _rest_client(inv, dummy_rest).set_discharge_target(4):
        ...
        setup_rest_case("0")
        if _rest_client(inv, dummy_rest).set_discharge_target(4):
            print("ERROR: {}: a write that never reads back must not be reported as successful".format(test_name))
            failed = True
        ...
        inv.base.had_errors = False
        _rest_client(inv, dummy_rest).set_discharge_target(4)
        ...
        inv.base.had_errors = False
        _rest_client(inv, dummy_rest).set_discharge_target(10)
        if not wrote_target(dummy_rest.get_commands()):
