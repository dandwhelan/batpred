        ...
        for month in range(1, 13):
            ...
            if self.heat_model:
                ...
                for day, weight in samples:
                    ...
                    try:
                        result = run_day(self.predbat, self.config, self.weather, self.tariff, self.gas_load_source or self.load_source, day, midnight_utc, plans=day_plans, baseline_tariff=self.baseline_tariff if baseline_ready else None)
                        ...
                        # Same baseline tariff as the gas leg above: the two legs' no_pvbat
                        # scenarios are differenced against each other, so pricing one on the
                        # baseline tariff and the other on the main tariff would bias the
                        # heating answer by whatever the two tariffs differ by.
                        heat_result = run_day(self.predbat, self.config, self.weather, self.tariff, self.heat_load_source, day, midnight_utc, baseline_tariff=self.baseline_tariff if baseline_ready else None)
                ...
                # No heat pump: upstream's plain per-month planner, which fast mode (gated off
                # above whenever a heat pump is configured) may later skip in favour of
                # interpolating this month from anchor months.
