...
class AnnualPredictor:
    ...
    async def _plan_months(self, tariff, year, zone, months_to_plan, progress, total_units, completed=0):
        ...
        for month in sorted(set(months_to_plan) | set(self.config["months"])):
            ...
            if self.heat_model:
                # Two legs - gas house, then heat-pump house - over the same sampled days.
                row = await self._plan_one_month_heat(tariff, month, year, zone, days_in_month, standing_charge_p, baseline_ready)
            else:
    ...
    async def run(self, progress=None):
        ...
        # Fast mode interpolates months from a handful of sampled anchors, which has no notion of a
        # heat pump's own month-by-month demand curve - forcing a full run keeps every heat-pump
        # month genuinely simulated rather than guessed from anchors that never modelled it.
        fast_mode = self.config["fast_mode"] and not self.heat_model
    ...
    async def _plan_one_month_heat(self, tariff, month, year, zone, days_in_month, standing_charge_p, baseline_ready):
        """Plan one month TWICE - house on gas, then house on the heat pump - and return its row.

        The heat-pump parallel of ``_plan_one_month``, kept as its own method for the same
        reason that one exists: ``_plan_months`` then branches on one line rather than
        carrying two loop bodies.

        Both legs run the SAME sampled days inside ONE try, because the heating answer is
        the difference between them - a day that succeeded in one leg but failed in the
        other would silently corrupt it, so a failure drops the day from both.

        Takes ``tariff`` explicitly for the same reason ``_plan_one_month`` does: an export
        sweep prices each day on ITS export product, and reading ``self.tariff`` here would
        quietly re-price every swept tariff against the main one.
        """
        samples = select_samples(self.weather, year, month, self.config["samples_per_month"], has_solar=bool(self.config["solar"]), sampling=self.config["sampling"])
        if not samples:
            return {"month": month, "status": "unavailable", "reason": "no usable weather days", "days": days_in_month, "standing_charge_p": standing_charge_p}
        ...
        for day, weight in samples:
            ...
            day_plans = [] if self.config["debug"] else None
            try:
                result = run_day(self.predbat, self.config, self.weather, tariff, self.gas_load_source or self.load_source, day, midnight_utc, plans=day_plans, baseline_tariff=self.baseline_tariff if baseline_ready else None)
                # A smart cylinder charges on the cheapest half-hours, so it has to see this
                # day's rates before its load profile is built. Set here rather than inside
                # prepare_sample() because prepare_sample builds the load forecast BEFORE it
                # fetches rates, and because the gas leg above must never see them - only the
                # heat pump leg has a cylinder to schedule.
                rate_import, _ = tariff.rates_for(midnight_utc, PLAN_MINUTES)
                self.heat_model.set_plan_rates(day, rate_import)
                # Same baseline tariff as the gas leg above: the two legs' no_pvbat scenarios
                # are differenced against each other, so pricing one on the baseline tariff
                # and the other on the main tariff would bias the heating answer by whatever
                # the two tariffs differ by.
                heat_result = run_day(self.predbat, self.config, self.weather, tariff, self.heat_load_source, day, midnight_utc, baseline_tariff=self.baseline_tariff if baseline_ready else None)
        ...
        def build_scenarios(scenario_totals):
            ...
            ``export_credit_p_estimate`` is an approximation, not a second income stream:
            cost_p already prices export at the real per-minute export rate for every minute
            it happened, so the export credit is already inside it. This is a cruder second
            estimate of the same money (a single day's flat average export rate), kept only
            for a human-readable "how much of that came from export" figure. Adding it to
            cost_p double-counts the export income - see the results-document caveat.
            """
            built = {}
            for key in SCENARIO_KEYS:
        ...
        row = {
            ...
            "scenarios": scenarios,
            # Which sides of *this* month's rates (if any) came from the tariff's
            # current-rates fallback rather than a real historical download - carried onto
            # the row itself, not just into a run-wide caveat, so a synthesised month is not
            # indistinguishable from a real one in the JSON, the chart or the table.
            "rates_synthesised": self._synthesised_sides(tariff.fallback_months, year, month),
        }
        if heat_day_results:
