...
def run_web_power_flow_tests(my_predbat):
    ...
    # -------------------------------------------------------------------------    # These render-level tests were written upstream against its circle-and-arrow diagram.
    # This fork draws an isometric house scene instead (web.py get_power_flow_diagram), so the
    # assertions below check the same behaviour - is the car drawn, is its arm animated, which
    # node feeds it, and what the House pill reads - against this fork's markup.
    ...
    html = web.get_power_flow_diagram()
    if 'class="pf-car"' in html:
        print("  ERROR: the car arm should not be drawn when no car charging power sensor is configured")
        failed += 1
    if ">3000 W<" not in html:
        print("  ERROR: the House pill should show the full load power when there is no car to subtract")
        failed += 1
    ...
    my_predbat.car_charging_power_configured = True
    html = web.get_power_flow_diagram()
    if 'class="pf-car"' not in html:
        print("  ERROR: expected a car arm in the diagram once a car charging power sensor is configured")
        failed += 1
    if ">2000 W<" not in html:
        print("  ERROR: expected the car charging power to be labelled on its pill")
        failed += 1
    if ">1000 W<" not in html:
        print("  ERROR: expected the House pill to show the load remainder (3000 - 2000) once the car is drawn separately")
        failed += 1
    if 'class="pf-flow pf-car"' not in html:
        print("  ERROR: expected the car arm to be animated while it is charging")
        failed += 1

    # -------------------------------------------------------------------------
    print("Test: a configured but idle charger keeps its arm, unanimated")
    my_predbat.car_charging_power = 0
    my_predbat.car_charging_power_configured = True
    html = web.get_power_flow_diagram()
    if 'class="pf-car"' not in html:
        ...
        failed += 1
    if 'class="pf-flow pf-car"' in html:
        print("  ERROR: the idle car arm should be a static guide line, as the PV arm is when not generating")
        failed += 1
    if ">3000 W<" not in html:
        print("  ERROR: the House pill should show the full load power when the car is drawing nothing")
        failed += 1
    ...
    my_predbat.car_charging_power_configured = True
    html = web.get_power_flow_diagram()
    if "House load 0 watts" not in html:
        print("  ERROR: expected the House remainder to clamp at 0 watts rather than go negative")
        failed += 1
    if "-2000 W" in html:
        print("  ERROR: the House pill must never show a negative load")
        failed += 1
    ...
    my_predbat.car_charging_power_configured = True
    html = web.get_power_flow_diagram()
    if "House load 542 watts" not in html:
        print("  ERROR: expected the House pill to keep its whole 542 W with the car outside the CT clamp")
        failed += 1
    if ">7363 W<" not in html:
        print("  ERROR: expected the car charging power to still be labelled on its own arm")
        failed += 1
    if "fed from the grid" not in html:
    ...
    html = web.get_power_flow_diagram()
    if "House load 542 watts" not in html:
        print("  ERROR: expected 7905 - 7363 = 542 W in the House pill with the car inside the CT clamp")
        failed += 1
    if "fed from the house" not in html:
